...
dbRDA_vector_scores <- scores(dbRDA_full_model, display = "bp", choices = 1:2) %>%
  ...
  mutate(
    ...
    label_x = case_when(
      variable == "mean_temperature_c_scaled" ~ 0.38,
      variable == "mean_salinity_psu_scaled" ~ 0.45,
      ...
      variable == "seafloor_depth_m_scaled" ~ 0.65,
      variable == "start_latitude_dd_scaled" ~ -0.49,
      TRUE ~ base_label_x
    ),
    label_y = case_when(
      variable == "mean_temperature_c_scaled" ~ -0.78,
      variable == "mean_salinity_psu_scaled" ~ -0.11,
      variable == "year2019" ~ -0.67,
      variable == "year2022" ~ 0.81,
      variable == "solar_dayness_scaled" ~ 0.54,
      variable == "dissolved_oxygen_ml_l_scaled" ~ -0.17,
      variable == "year2023" ~ -0.1,
      variable == "mean_chl_0_100_m_mgm3_scaled" ~ -0.18,
      variable == "depth_mean_m_scaled" ~ 0.22,
      variable == "seafloor_depth_m_scaled" ~ 0.05,
      variable == "start_latitude_dd_scaled" ~ 0.68,
      TRUE ~ base_label_y
    ),
    label_hjust = case_when(
      variable %in% c("mean_temperature_c_scaled", "mean_salinity_psu_scaled",
                      "depth_mean_m_scaled", "seafloor_depth_m_scaled",
                      "year2019", "year2018", "year2022") ~ 0,
      variable %in% c("dissolved_oxygen_ml_l_scaled", "solar_dayness_scaled", 
                      "year2023", "mean_chl_0_100_m_mgm3_scaled",
                      "start_latitude_dd_scaled") ~ 1,
      CAP1 >= 0 ~ 0,
      TRUE ~ 1
    )
  )
