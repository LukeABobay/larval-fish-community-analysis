...
d_n_scatter_main_clust <- ggplot(main_clust_model_data, aes(x = depth_mean_m, y = log(individuals_per_m3), color = taxon)) +
  ...
  labs(title = "Taxa concentrations by mean depths for main cluster samples",
       x = "Mean tow depth (m)", y = "Concentration (log(ind./m3)", color = "Taxon") +
  theme_classic()
ggsave("solar_dayness_scatter_main_clust.png", plot = d_n_scatter_main_clust, path = here("output"),
       width = 7, height = 5, units = "in", dpi = 300)


# Fit linear model(s) of taxa concentrations against depth and solar dayness ---
main_clust_solar_dayness_depth_model <- lm(avg_taxa_concentration ~ taxon*solar_dayness + taxon*depth_range, data = main_clust_avgd_mocness_major_taxa_19)
summary(main_clust_solar_dayness_depth_model)

main_clust_solar_dayness_mean_depth_model <- lm(avg_taxa_concentration ~ taxon*solar_dayness + taxon*depth_mean_m, data = main_clust_avgd_mocness_major_taxa_19)
summary(main_clust_solar_dayness_mean_depth_model)


# Model without allowing effects of depth and solar dayness to vary among taxa
main_clust_simple_model_pois <- glm(individuals_in_tow ~ taxon + solar_dayness + depth_mean_scaled + offset(log(volume_best_m3_both_sides)),
                         family = poisson,
...
main_clust_full_model_pois <- glmmTMB(individuals_in_tow ~ taxon * solar_dayness * depth_mean_scaled +
...
main_clust_full_model_nb <- glmmTMB(individuals_in_tow ~ taxon * solar_dayness * depth_mean_scaled +
                           offset(log(volume_best_m3_both_sides)) +
                           (1 | transect_station_rep_year_net),
                         family = nbinom2,
                         data = main_clust_model_data)
...
main_clust_full_model_nb_terms <- terms(~ taxon * solar_dayness * depth_mean_scaled)
...
main_clust_full_model_nb_effect_tests <- map_dfr(levels(main_clust_full_model_nb_effects$taxon), function(current_taxon) {
  ...
  depth_dayness_p <- main_clust_full_model_nb_wald_test(rows[c(1, 4), ], rows[c(2, 3), ])
  
  tibble(taxon = factor(current_taxon, levels = levels(main_clust_model_data$taxon)),
         depth_dayness_p = depth_dayness_p)
}) %>%
  mutate(label = paste0("Dayness-depth interaction: ", main_clust_full_model_nb_p_label(depth_dayness_p))) %>%
...
ggsave(here("output/main_clust_full_model_nb_solar_dayness_depth_effects.png"),
...
ggplot(sp_of_interest, aes(x = depth_mean_m, y = log(individuals_per_m3), color = taxon, fill = taxon)) +
  ...
  theme_classic()
ggsave("main_clust_solar_dayness_scatter_sp_of_interest.png", plot = get_last_plot(), path = here("output"),
       width = 7, height = 5, units = "in", dpi = 300)
...
main_clust_seb_lm <- glmmTMB(individuals_in_tow ~ solar_dayness * depth_mean_scaled + seafloor_depth_scaled +
...
visreg(main_clust_seb_lm, "solar_dayness",
...
main_clust_p_vetulus_lm <- glmmTMB(individuals_in_tow ~ solar_dayness * depth_mean_scaled + seafloor_depth_scaled +
                          mean_temperature_scaled + mean_salinity_scaled + dissolved_oxygen_scaled + mean_chl_0_100_m_scaled +
                          offset(log(volume_best_m3_both_sides)),
                        family = nbinom2,
...
visreg(main_clust_p_vetulus_lm, "solar_dayness",
...
main_clust_s_leucopsarus_lm <- glmmTMB(individuals_in_tow ~ solar_dayness * depth_mean_scaled + seafloor_depth_scaled +
                              mean_temperature_scaled + mean_salinity_scaled + dissolved_oxygen_scaled + mean_chl_0_100_m_scaled +
                              offset(log(volume_best_m3_both_sides)),
                            family = nbinom2,
...
visreg(main_clust_s_leucopsarus_lm, "solar_dayness",
...
main_clust_ammodytidae_lm <- glmmTMB(individuals_in_tow ~ solar_dayness * depth_mean_scaled + seafloor_depth_scaled +
                           mean_temperature_scaled + mean_salinity_scaled + dissolved_oxygen_scaled + mean_chl_0_100_m_scaled +
                           offset(log(volume_best_m3_both_sides)),
                         family = nbinom2,
...
visreg(main_clust_ammodytidae_lm, "solar_dayness",
